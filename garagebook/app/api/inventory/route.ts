...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = q.from('inventory').select('*');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: exists } = await q.from('inventory').select('id').ilike('name', name.trim()).ilike('company', company?.trim() || '').maybeSingle();
    if (exists) return apiError(`"${name.trim()}" pehle se exist karta hai`, 409);

    if (sku?.trim()) {
      const { data: skuExists } = await q.from('inventory').select('id').eq('sku', sku.trim()).maybeSingle();
      if (skuExists) return apiError('Ye SKU pehle se use ho raha hai', 409);
    }

    const { data, error } = await q.from('inventory').insert({
