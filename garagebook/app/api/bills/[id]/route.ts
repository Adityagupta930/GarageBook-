...
export async function GET(_: NextRequest, { params }: Params) {
  try {
    ...
    const { data: bill, error } = await q.from('bills').select('*').eq('id', id).single();
    if (error || !bill) return apiError('Bill nahi mila', 404);
    const { data: items } = await q.from('bill_items').select('*').eq('bill_id', id);
...
export async function DELETE(_: NextRequest, { params }: Params) {
  try {
    const { id } = await params;
    const { error } = await q.from('bills').delete().eq('id', id);
